/* ============================================================
   Peluquería Pro · Frontend SPA (vanilla, sin dependencias)
   Rediseño UX/UI v2 — solo frontend; contratos de API intactos.
   ============================================================ */
(() => {
  'use strict';
  ...
  const state = {
    ...
    staffFilter: 'all',
    statusFilter: 'all',
    customers: [], services: [], staff: [], appointments: [], items: [], documents: [],
    dashboard: null,
    loaded: { customers: false, services: false, staff: false, appointments: false, items: false, documents: false },
    busy: false,
    prevFocus: null,
  };
  ...
  async function enterApp() {
    ...
    document.title = `${tenant.name} · Peluquería Pro`;
    await refreshAll();
  ...
  const VIEW_META = {
    dashboard: ['Inicio', 'Resumen del negocio hoy'],
    agenda: ['Agenda', 'Citas de la semana'],
    clientes: ['Clientes', 'Tus clientes registrados'],
    servicios: ['Servicios', 'Catálogo de servicios'],
    empleados: ['Empleados', 'Equipo del negocio'],
  ...
  const ORDER = ['dashboard', 'agenda', 'clientes', 'servicios', 'empleados', 'inventario', 'documentos', 'config'];
  ...
  /* ---------- Dashboard (usa /api/dashboard/summary, ya existente) ---------- */
  ...
  function fillDashboard() {
    ...
    const cards = [
      { icon: 'calendar', label: 'Citas hoy (confirmadas)', value: summary.appointmentsToday, cls: 'ok', hint: 'programadas para esta jornada' },
      { icon: 'users', label: 'Clientes', value: summary.customers, cls: '', hint: 'en tu lista' },
      { icon: 'box', label: 'Productos bajos stock', value: summary.lowStockItems, cls: summary.lowStockItems > 0 ? 'danger' : '', hint: summary.lowStockItems > 0 ? 'requieren reposición' : 'sin faltantes' },
    ];
    ...
    if (!upcoming.length) {
      $('#dash-upcoming').innerHTML = `<div class="empty-state"><span class="empty-icon">${svg('clock', 24)}</span><h4>Sin citas próximas</h4><p>Agenda una nueva cita para empezar.</p><button class="btn primary sm" data-empty-action="new-appt" type="button">${svg('plus', 15)} Agendar cita</button></div>`;
    ...
    $('#dash-upcoming').innerHTML = upcoming.map((a) => {
      ...
      return `<button class="up-item" type="button" data-go-agenda>
        <span class="up-time">${fmtDay(start)}</span>
        <span class="up-body"><b>${esc(custMap.get(a.customer_id) || 'Cliente')}</b><small>${esc(svc)}</small></span>
        <span class="tag ${a.status}">${STATUS_LABEL[a.status] || a.status}</span>
        <span class="up-arrow">${svg('arrowRight', 16)}</span>
      </button>`;
    }).join('');
    $('#dash-upcoming').innerHTML += `<div style="padding:10px 20px;border-top:1px solid var(--border)"><button class="btn ghost sm" data-go-agenda type="button">Ver todas en la agenda</button></div>`;
    $$('[data-go-agenda]').forEach((b) => b.addEventListener('click', () => switchPane('agenda')));
  }
  const fmtDay = (d) => {
  ...
  function openDialogFor(action) {
    switch (action) {
      ...
      case 'new-staff': openNewStaff(); break;
      ...
      case 'reload-staff': renderEmpleados(); break;
  ...
  async function renderAgenda() {
    ...
    if (state.staffFilter !== 'all') filtered = filtered.filter((a) => a.staff_id === state.staffFilter);
      ...
      for (const [mm, min] of [[0, 0], [30, 1]]) {
        ...
        for (const i of days) {
          ...
          for (const a of dayApps) {
            ...
            const color = staff?.color || '#292524';
            ...
            row += `<div class="agenda-slot ${a.status}" data-appt="${a.id}" tabindex="0" role="button"
                aria-label="${esc(cust?.name || 'Cliente')}, ${esc((a.services || []).map((s) => s.name).join(', ')) || 'servicio'}, ${timeLabel}, ${STATUS_LABEL[a.status] || a.status}"
              ...
              <b>${esc(cust?.name || '?')}</b>
              <span class="meta">${esc((a.services || []).map((s) => s.name).join(', ') || '')}</span>
    ...
    $('#agenda-caption').innerHTML = dayAppsTotal.length
      ? `Citas programadas esta semana: <b>${filtered.length}</b>${filtered.length !== dayAppsTotal.length ? ' (con filtros)' : ''} · Citas de hoy: <b>${hoyCount}</b>`
      : 'Sin citas programadas en esta semana. Haz clic en una franja horaria para agendar.';

    buildLegend(staffMap);
    bindAgendaCells();
    bindSlots();
    updateDayLabel();
  }
  ...
  function buildStaffFilter() {
    const sel = $('#agenda-staff');
    const cur = sel.value;
    const opts = ['<option value="all">Todos los empleados</option>']
      .concat(state.staff.map((s) => `<option value="${s.id}" ${s.id === cur ? 'selected' : ''}>${esc(s.name)}</option>`))
      .join('');
    sel.innerHTML = opts;
  }

  function buildLegend(staffMap) {
    const chips = [...staffMap.values()].filter((s) => s.active).map((s) =>
      `<span class="legend-chip"><i style="--b:${s.color}CC;--c:${s.color}"></i>${esc(s.name)}</span>`);
  ...
  /* Filtros */
  $('#agenda-staff').addEventListener('change', (e) => { state.staffFilter = e.target.value; renderAgenda(); });
  ...
  function pickHtml() {
    ...
    const staff = state.staff.filter((s) => s.active).map((s) => `<option value="${s.id}">${esc(s.name)}</option>`).join('');
    ...
    return `
      <form class="modal-form" novalidate>
        <div class="form-row">
          ...
          <label class="field"><span>Empleado</span><select name="staffId">${`<option value="">Sin asignar</option>`}${staffOpts}</select></label>
        </div>
        <label class="field"><span>Servicios (Ctrl+clic para varios)</span>
        ...
        <div class="form-row">
          ...
          <label class="field"><span>Duración (min)</span><input type="number" name="durationMin" value="${a ? durMinutes(a) : 30}" min="5" max="600" required /></label>
        </div>
        ...
        <div class="row-actions">
          ...
          <button type="submit" class="btn primary">${a ? 'Guardar cambios' : 'Agendar cita'}</button>
        </div>
      </form>`;
  }
  ...
  function openEditAppointment(id) {
    ...
    if (del) del.addEventListener('click', async () => {
      const ok = await confirmDialog({ title: 'Eliminar cita', message: `¿Eliminar la cita de la agenda? Esta acción no se puede deshacer.`, danger: true });
      if (!ok) return;
  ...
  function bindAppointmentForm(existing) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const input = {
        ...
        durationMin: Math.max(5, Number(f.get('durationMin')) || 30),
  ...
  async function renderClientes() {
    ...
    if (!state.customers.length) {
      tbody.innerHTML = `<tr><td colspan="5">${emptyStateHtml('Aún no tienes clientes', 'Registra tu primer cliente para agendar citas.', 'Registrar cliente', 'new-customer', 'userPlus')}</td></tr>`;
      bindEmptyActions(tbody);
      return;
    }
  ...
  async function renderServicios() {
    ...
    if (!state.services.length) {
      $('#servicios-tbody').innerHTML = `<tr><td colspan="5">${emptyStateHtml('Aún no tienes servicios', 'Agrega tu primer servicio para poder agendar citas.', 'Crear servicio', 'new-service', 'scissors')}</td></tr>`;
    ...
    $('#servicios-tbody').innerHTML = state.services.map((s) => `<tr>
      ...
      <td data-label="Duración">${s.durationMin} min</td>
  ...
  function serviceFormHtml(s) {
    return `<form class="modal-form" novalidate>
      ...
      <div class="form-row">
        <label class="field"><span>Duración (min)</span><input name="durationMin" type="number" value="${s?.durationMin ?? 30}" min="5" required /></label>
  ...
  async function renderEmpleados() {
    ...
    try {
      ...
      tbody.innerHTML = `<tr><td colspan="5">${emptyStateHtml('No se pudo cargar', e.message, 'Reintentar', 'reload-staff')}</td></tr>`;
      bindEmptyActions(tbody);
      return;
    }
    const svcMap = new Map(state.services.map((s) => [s.id, s.name]));
    if (!state.staff.length) {
      $('#empleados-tbody').innerHTML = `<tr><td colspan="5">${emptyStateHtml('Aún no tienes empleados', 'Agrega a tu equipo para organizar las citas por persona.', 'Crear empleado', 'new-staff', 'user')}</td></tr>`;
      bindEmptyActions($('#empleados-tbody'));
      return;
    }
    $('#empleados-tbody').innerHTML = state.staff.map((s) => {
      const skills = (s.skillIds || []).map((id) => svcMap.get(id)).filter(Boolean);
      return `<tr>
        <td><span class="avatar-sm" style="background:${s.color}55"></span><b>${esc(s.name)}</b></td>
        <td data-label="Teléfono">${esc(s.phone || '—')}</td>
        <td data-label="Servicios" class="muted">${esc(skills.join(', ') || 'Sin especialidad')}</td>
        <td data-label="Estado"><span class="tag ${s.active ? 'confirmed' : 'off'}">${s.active ? 'Activo' : 'Inactivo'}</span></td>
        <td data-label="Acciones"><div class="row-actions">
          <button class="btn ghost sm" data-edit="${s.id}" type="button" aria-label="Editar ${esc(s.name)}">${svg('edit', 15)} Editar</button>
          <button class="btn danger sm" data-del="${s.id}" type="button" aria-label="Eliminar ${esc(s.name)}">${svg('trash', 15)}</button>
  ...
  function staffFormHtml(s) {
    const skills = state.services.filter((x) => x.active).map((x) =>
      `<label class="check"><input type="checkbox" name="skillIds" value="${x.id}" ${s?.skillIds?.includes(x.id) ? 'checked' : ''}/> <span>${esc(x.name)}</span></label>`).join('');
    return `<form class="modal-form" novalidate>
      <label class="field"><span>Nombre</span><input name="name" value="${esc(s?.name || '')}" required /></label>
      <div class="form-row">
        <label class="field"><span>Teléfono</span><input name="phone" value="${esc(s?.phone || '')}" /></label>
        <label class="field"><span>Color en agenda</span><input name="color" type="color" value="${s?.color || '#6d28d9'}" /></label>
      </div>
      <fieldset class="divide-nicely"><legend>Servicios que realiza</legend>${skills || '<span class="muted">Crea servicios primero</span>'}</fieldset>
      <label class="field"><span>Estado</span><select name="active"><option value="true" ${s?.active === false ? '' : 'selected'}>Activo</option><option value="false" ${s?.active === false ? 'selected' : ''}>Inactivo</option></select></label>
      <div class="row-actions">
        <button type="button" class="btn ghost" data-close>Cancelar</button>
        <button type="submit" class="btn primary">${s ? 'Guardar' : 'Agregar'}</button>
  ...
  function openNewStaff() {
    ...
    bindSimpleForm('#modal .modal-form', '/api/staff', 'POST', renderEmpleados);
  }
  ...
  async function renderInventario() {
    ...
    if (!state.items.length) {
      $('#inventario-tbody').innerHTML = `<tr><td colspan="7">${emptyStateHtml('Sin artículos en inventario', 'Registra productos para controlar el stock del negocio.', 'Agregar artículo', 'new-item', 'box')}</td></tr>`;
  ...
  function itemFormHtml(i) {
    return `<form class="modal-form" novalidate>
      ...
      <div class="form-row">
        ...
        <label class="field"><span>Unidad</span><input name="unit" value="${esc(i?.unit || 'unidad')}" /></label>
      </div>
      <div class="form-row">
  ...
  function openMovement(id) {
    ...
    openModal(`Ajustar stock · ${item.name}`, `<form class="modal-form" novalidate>
      ...
      <label class="field"><span>Motivo</span><input name="reason" placeholder="reposición, venta, merma…" required /></label>
      <div class="row-actions">
        <button type="button" class="btn ghost" data-close>Cancelar</button>
  ...
  function bindMovementForm(id) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (delta < 0) {
        toast('Las salidas de stock aún no están disponibles en esta versión', 'error');
        return;
      }
      try {
  ...
  function openNewDocument() {
    ...
    openModal('Nueva cotización', `<form class="modal-form" id="doc-form" novalidate>
      ...
      <label class="field"><span>Título</span><input name="title" placeholder="Corte + tratamiento" /></label>
      <div id="doc-lines">
  ...
  $('#empleados-new').addEventListener('click', openNewStaff);
  ...
  async function refreshAll() {
    ...
    await Promise.all([
      ...
      renderEmpleados().then(() => { state.loaded.staff = true; }),
